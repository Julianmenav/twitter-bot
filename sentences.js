...
const getSentence = (totalGames, wins, twitterName) => {
  ...
  if (totalGames > 5) {
    switch (true) {
      case (winRatio < 0.25):
        sentence = [`Señoras y señores, ${mention} ES la pochiqueue`, `.${mention} no debería haber jugado hoy`, `x9 ${mention} for int please`]
        break;
      case (winRatio < 0.45):
        sentence = [`Parece que ${mention} va en silla de ruedas`, `Mal día para ${mention}`, `.${mention} esta troleando`]
        break;
      case (winRatio < 0.6):
    ...
    switch (true) {
      case (winRatio < 0.25):
        sentence = [`A ${mention} no le estan saliendo las cosas`, `Parece que ${mention} va en silla de ruedas`]
        break;
      case (winRatio < 0.45):
